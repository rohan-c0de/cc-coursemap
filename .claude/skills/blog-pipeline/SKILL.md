...
- ⚠️ Lightly covered (hub-only): course-explainer (1 hub, no spokes yet — see #368 for the GSC-driven detector that will produce per-course-code spokes once built)
...
| `course-explainer-guide` ✅ hub live (2026-05-11), detector pending | (hub: `community-college-course-codes-explained`) | new: `detect-course-explainer-demand.ts` (pending) reads `~/gsc_audit_output.json` for course-code-shaped queries with high impressions + low CTR; emits a candidate per high-demand course code with state + college identified from quoted phrases. **First detector that reads search demand, not data presence.** | §6 College-specific | [#368](https://github.com/rohan-c0de/cc-coursemap/issues/368) |
