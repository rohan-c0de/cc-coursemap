...
| `course-explainer-guide` ✅ hub + detector + 1 spoke live (2026-05-11) | (hub: `community-college-course-codes-explained`) | ✅ `detect-course-explainer-demand.ts` reads `~/gsc_audit_output.json` for course-code-shaped queries with high impressions + low CTR; emits a candidate per high-demand course code with state + college extracted from quoted phrases. **First detector that reads search demand, not data presence.** | §6 College-specific | [#368](https://github.com/rohan-c0de/cc-coursemap/issues/368) |
