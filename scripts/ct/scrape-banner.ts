/**
 * CT State Community College course scraper — Banner SSB 9 REST API
 * Base URL: https://reg-prod.ec.ct.edu
 *
 * Usage: npx tsx scripts/ct/scrape-banner.ts
 */
...
function bannerTermToStandard(code: string): string {
  // CT State: 202710 = Fall 2026, 202640 = Spring 2026, 202650 = Summer 2026
  const year = parseInt(code.substring(0, 4));
  const suffix = code.substring(4);
  if (suffix === "10") return `${year - 1}FA`; // Fall: 202710 -> 2026FA
  if (suffix === "40") return `${year}SP`; // Spring: 202640 -> 2026SP
  if (suffix === "50") return `${year}SU`; // Summer: 202650 -> 2026SU
  return `${year}XX`;
}
...
async function buildSubjectMap(termCode: string, cookies: string): Promise<void> {
  try {
    ...
    // Clear stale entries from previous terms
...
async function main() {
  ...
  const outDir = path.join(process.cwd(), "data", "ct", "courses", COLLEGE_SLUG);
  ...
  if (!process.argv.includes("--no-import")) {
    ...
    await importCoursesToSupabase("ct");
  }
